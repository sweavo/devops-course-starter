...
class MarvelSession(object):
    ...
    def request_url(self, path, query_dict):
        ...
        query_items = {"ts": str(ts), "hash": hash_string, "apikey": self._public_key}
...
def configure_argument_parsing():
    ...
    ap.add_argument(
        ...
        default="",
        nargs="?",
        help="The first part of the name of a character. If omitted, all characters will be listed.",
    )
    return ap
...
if __name__ == "__main__":
    ...
    characters, attribution = retrieve_characters_by_part_name(
        arguments.SEARCH_TERM, progress
    )
